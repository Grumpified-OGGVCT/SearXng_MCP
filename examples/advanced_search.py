...
async def main():
    ...
    data = json.loads(results) if isinstance(results, str) else results
    print(f"Query: künstliche intelligenz :de")
    print(f"Results: {len(data.get('results', []))} found")
    ...
    data = json.loads(results) if isinstance(results, str) else results
    print(f"Query: machine learning frameworks")
    print(f"Categories: it,science")
    print(f"Results: {len(data.get('results', []))} found")
    ...
    data = json.loads(results) if isinstance(results, str) else results
    print(f"Query: async python best practices")
    print(f"Engines: github,stackoverflow")
    print(f"Results: {len(data.get('results', []))} found")
    ...
    data = json.loads(results) if isinstance(results, str) else results
    print(f"Query: transformer architecture")
    print(f"Category: science")
    print(f"Engine: arxiv")
    print(f"Time range: month")
    print(f"Results: {len(data.get('results', []))} found")
    ...
    data = json.loads(results) if isinstance(results, str) else results
    print(f"Query: educational videos")
    print(f"Category: videos")
    print(f"Safe search: 2 (strict)")
    print(f"Results: {len(data.get('results', []))} found")
    ...
    data = json.loads(results) if isinstance(results, str) else results
    print(f"Query: 百度搜索 (Baidu Search)")
    print(f"Engine: baidu")
    print(f"Language: zh")
    print(f"Results: {len(data.get('results', []))} found")
