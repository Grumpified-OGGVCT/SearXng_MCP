...
class RateLimiter:
    ...
    def __init__(self, custom_limits: dict[str, int] | None = None):
        ...
        self.request_history: dict[str, deque] = {provider: deque() for provider in self.limits}
    ...
    def check_rate_limit(self, provider: str) -> tuple[bool, float | None]:
        """
    ...
    def get_status(self, provider: str | None = None) -> dict:
        ...
        if provider:
            ...
            # All providers
            return {provider: self.get_status(provider) for provider in self.limits}
