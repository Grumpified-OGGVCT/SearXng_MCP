...
class MetricsCollector:
    ...
    def record_search(
        ...
        token_estimate: Optional[Dict[str, int]] = None,
    ):
        """
    ...
    def _estimate_cost(
        self, provider: str, model: str, token_estimate: Dict[str, int]
    ) -> float:
        """
    ...
    def get_session_metrics(self) -> Dict[str, Any]:
        ...
        for provider, metrics in self.session_metrics["providers"].items():
            ...
            avg_latency = (
                metrics["total_latency"] / requests if requests > 0 else 0
            )
            success_rate = (
                metrics["successes"] / requests * 100 if requests > 0 else 0
            )
        ...
        try:
            ...
            if existing_metrics:
                ...
                for key in ["total", "search_only", "ai_enhanced", "cached"]:
                    existing_metrics["requests"][key] = existing_metrics["requests"].get(key, 0) + \
                                                       current_metrics["requests"][key]
    ...
    def get_recent_errors(self, count: int = 10) -> List[Dict[str, Any]]:
        ...
        # Format timestamps
        for error in errors:
            error["timestamp_formatted"] = datetime.fromtimestamp(
                error["timestamp"]
